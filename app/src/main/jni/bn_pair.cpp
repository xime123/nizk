...
void PFC::add_to_hash(char *x, int n)
{
	int i=0;
	for(i = 0; i < n; i++)
	{
...
Big PFC::random_to_hash(int n, const Big& x)
{
	Big result;
	start_hash();
	shs256_process(&SH, n);
	add_to_hash(x);
	result = finish_hash_to_group();
	return result;
}
...
/******/
G1 PFC::multn(int n, Big* y, G1* point)
{
	G1 result;
	ECn *Px;
	int i;
...
GT PFC::power2(const GT& gt,const Big& b)
{
	GT result;
	ZZn12 tmp;
	ZZn12 r;
...
GT PFC::powern(int n, Big *y, GT *point)
{
	GT result;
...
PFC::PFC(int s, csprng *rng, BYTE *pbPara)
{
	...
	*mod=36*pow(X,4)+36*pow(X,3)+24*X*X+6*X+1;//p
	*trace=6*X*X+1;//t
	*npoints=*mod+1-*trace;//r
	*cof=*mod-1+*trace;
	*ord=*npoints;
...
// Use Galbraith & Scott Homomorphism idea for G2 & GT ... (http://eprint.iacr.org/2008/117.pdf EXample 5)
	...
	*BB[3][0]=X-1;   *BB[3][1]=4*X+2; *BB[3][2]=-(2*X-1); *BB[3][3]=X-1;
	...
	if (pbPara !=NULL)
	{
		*g_1 = bytes_to_g1(pbPara);//g1
		*g_2 = bytes_to_g2(pbPara + 2*bytes_per_big);//g2
		*g_t = bytes_to_gt(pbPara + 6*bytes_per_big);//gt
		*h = bytes_to_g1(pbPara + 18*bytes_per_big);//h
	}
...
G1 PFC::bytes_to_g1(BYTE *bytes, int bytelen)
{
	G1 result;
	ECn tmp;
	Big x,y;
	int bytes_per_big=(MIRACL/8)*(get_mip()->nib-1);
	x = from_binary(bytes_per_big, (char*)bytes);
	y = from_binary(bytes_per_big, (char*)(bytes+bytes_per_big));
	tmp.set(x,y);
	result.mtable=NULL;
	result.mtbits=0;
	result.g = tmp;
...
G2 PFC::bytes_to_g2(BYTE *bytes, int bytelen )
{
	G2 result;
	ZZn2 x, y;
	Big a, b;
	int bytes_per_big=(MIRACL/8)*(get_mip()->nib-1);
	a = from_binary(bytes_per_big, (char*)bytes);
	b = from_binary(bytes_per_big, (char*)(bytes + bytes_per_big));
	x.set(a,b);
	a = from_binary(bytes_per_big, (char*)(bytes + 2*bytes_per_big));
	b = from_binary(bytes_per_big, (char*)(bytes + 3*bytes_per_big));
	y.set(a,b);
	result.ptable=NULL;
	result.mtable=NULL;
	result.mtbits=0;
...
GT PFC::bytes_to_gt(BYTE *bytes, int bytelen)
{
	GT result;
	ZZn4 a,b,c;
	ZZn2 d,e;
	...
	y = from_binary(bytes_per_big, (char*)(bytes + bytes_per_big));
	d.set(x, y);
	x = from_binary(bytes_per_big, (char*)(bytes + 2*bytes_per_big));
	y = from_binary(bytes_per_big, (char*)(bytes + 3*bytes_per_big));
	e.set(x, y);
	a.set(d,e);
	///////////////////////////////
	x = from_binary(bytes_per_big, (char*)(bytes + 4*bytes_per_big));
	y = from_binary(bytes_per_big, (char*)(bytes + 5*bytes_per_big));
	d.set(x, y);
	x = from_binary(bytes_per_big, (char*)(bytes + 6*bytes_per_big));
	y = from_binary(bytes_per_big, (char*)(bytes + 7*bytes_per_big));
	e.set(x, y);
	b.set(d, e);
	/////////////////////
	x = from_binary(bytes_per_big, (char*)(bytes + 8*bytes_per_big));
	y = from_binary(bytes_per_big, (char*)(bytes + 9*bytes_per_big));
	d.set(x, y);
	x = from_binary(bytes_per_big, (char*)(bytes + 10*bytes_per_big));
	y = from_binary(bytes_per_big, (char*)(bytes + 11*bytes_per_big));
	e.set(x, y);
	c.set(d, e);
...
void GT::GT_To_Bytes(BYTE *bytes)
{
	ZZn4 a,b,c;
	ZZn2 d,e;
	Big x,y;
	...
	a.get(d,e);
	d.get(x,y);
	to_binary(x, bytes_per_big, (char*)bytes, true);
	to_binary(y, bytes_per_big, (char*)(bytes+ bytes_per_big), true);
	e.get(x,y);
	to_binary(x, bytes_per_big, (char*)(bytes+ 2*bytes_per_big), true);
	to_binary(y, bytes_per_big, (char*)(bytes+ 3*bytes_per_big), true);

	b.get(d,e);
	d.get(x,y);
	to_binary(x, bytes_per_big, (char*)(bytes+ 4*bytes_per_big), true);
	to_binary(y, bytes_per_big, (char*)(bytes+ 5*bytes_per_big), true);
	e.get(x,y);
	to_binary(x, bytes_per_big, (char*)(bytes+ 6*bytes_per_big), true);
	to_binary(y, bytes_per_big, (char*)(bytes+ 7*bytes_per_big), true);

	c.get(d,e);
	d.get(x,y);
	to_binary(x, bytes_per_big, (char*)(bytes+ 8*bytes_per_big), true);
	to_binary(y, bytes_per_big, (char*)(bytes+ 9*bytes_per_big), true);
	e.get(x,y);
	to_binary(x, bytes_per_big, (char*)(bytes+ 10*bytes_per_big), true);
	to_binary(y, bytes_per_big, (char*)(bytes+ 11*bytes_per_big), true);
}
...
void G1::G1_To_Bytes(BYTE *bytes)
{
...
void G2::G2_To_Bytes(BYTE *bytes)
{
	ZZn2 x, y;
	Big a, b;
	int bytes_per_big=(MIRACL/8)*(get_mip()->nib-1);
	...
	x.get(a, b);
	to_binary(a, bytes_per_big, (char*)bytes, true);
	to_binary(b, bytes_per_big, (char*)(bytes + bytes_per_big), true);

	y.get(a, b);
	to_binary(a, bytes_per_big, (char*)(bytes + 2*bytes_per_big), true);
	to_binary(b, bytes_per_big, (char*)(bytes + 3*bytes_per_big), true);
...
int PFC::precomp_for_mult(G1& w,BOOL smaller)
{
	ECn v=w.g;
	int i,j,k,bp,is,t;
	if (smaller) t=MR_ROUNDUP(2*S,WINDOW_SIZE);
...
int PFC::precomp_for_mult(G2& w,BOOL smaller)
{
	ECn2 v;
	ZZn2 x,y;
	int i,j,k,bp,is,t;
...
int PFC::precomp_for_power(GT& w,BOOL smaller)
{
	ZZn12 v=w.g;
	int i,j,k,bp,is,t;
	if (smaller) t=MR_ROUNDUP(2*S,WINDOW_SIZE);
